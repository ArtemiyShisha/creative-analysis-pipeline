...
def generate_recommendations(zones, total_zones_attention, background_attention):
    ...
    prompt = f"""Проанализируй результаты eye-tracking анализа рекламного креатива.
...
1. **Пойми контекст креатива** — что это за формат (баннер, карточка товара, промо), какую задачу он решает (awareness, конверсия, информирование), какой месседж передаёт.

2. **Оцени визуальную иерархию** — соответствует ли распределение внимания логике креатива? Взгляд должен следовать от главного к второстепенному в правильном порядке.

3. **Применяй принципы дизайна и UX:**
   - Визуальная иерархия: главное должно привлекать больше внимания
   - Путь взгляда: есть ли логичная последовательность восприятия элементов
   - Баланс: не перегружен ли креатив, есть ли "воздух"
   - Контраст и читаемость: выделяются ли ключевые элементы
   - Единство композиции: работают ли элементы вместе

4. **Не применяй жёсткие числовые критерии** — нет универсальных правил "CTA должна получать X%". Всё зависит от контекста и целей конкретного креатива.

---

**Результат анализа:**

1. **Overall Score (1-5)** — общая оценка эффективности визуальной коммуникации
2. **Reasoning** — краткое объяснение оценки с учётом контекста (2-3 предложения)
3. **Рекомендации (3-5 штук)** — конкретные улучшения на основе принципов дизайна:
   - priority: "High" / "Medium" / "Low"
   - title: короткий заголовок
   - description: подробное описание что и почему улучшить
  ...
  "recommendations": [
    ...
    payload = {
        ...
        'messages': [
            {
                'role': 'system',
                'content': 'Ты эксперт по визуальному дизайну, UX и рекламным коммуникациям. Ты анализируешь креативы с точки зрения принципов дизайна и эффективности визуальной коммуникации, а не по жёстким шаблонам.'
            },
            {
                'role': 'user',
                'content': prompt
